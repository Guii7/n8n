    ...
    def _launch_browser(self):
        ...
        try:
            ...
            browser_args = [
                ...
                '--start-minimized',
                # Evita interferência com input/clipboard do usuário
                '--disable-focus-stealing-prevention',
                '--disable-popup-blocking',
                '--no-first-run',
                '--no-default-browser-check',
                # Desabilita aceleração GPU para evitar conflitos
                '--disable-gpu',
                '--disable-software-rasterizer',
            ]

            # Se janela oculta está ativa, posiciona fora da tela e minimiza
            if self.browser_window_hidden:
                browser_args.extend([
                    '--window-position=-32000,-32000',  # Bem longe da tela
                    '--window-size=1,1',  # Janela mínima
                ])
        ...
        try:
            ...
            for batch_idx in range(0, len(product_urls), urls_per_batch):
                ...
                try:
                    ...
                    # IMPORTANTE: Usa JavaScript para definir valor diretamente
                    # Isso EVITA o clipboard (page.fill usa clipboard internamente!)
                    textarea_element = page.query_selector(textarea_selector)

                    # Limpa via JavaScript (não usa clipboard)
                    page.evaluate('(el) => { el.value = ""; el.dispatchEvent(new Event("input", { bubbles: true })); }', textarea_element)
                    time.sleep(0.3)

                    # Preenche via JavaScript (não usa clipboard)
                    page.evaluate('''(args) => {
                        const [el, text] = args;
                        el.value = text;
                    ...
                    # Usa input_value() que é API nativa do Playwright (não usa JavaScript evaluate)
